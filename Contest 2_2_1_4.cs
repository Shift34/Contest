Ваша задача построить минимальное остовное дерево графа методом с помощью алгоритма Прима и вывести последовательность ребер графа в порядке добавления их в дерево.
...
namespace ConsoleApp22
{
    internal class Program
    {
        public class Algorithm
        {
            ...
            {
                list.Add(graph.edge[0]);
                hashset.Add(graph.edge[0].firstpick);
                hashset.Add(graph.edge[0].secondpick);
                for (int i = 0; i < graph.countpicks; i++)
                {
                    for (int j = 0; j < graph.edge.Length;j++)
                    {
                        if (!hashset.Contains(graph.edge[j].firstpick)&& hashset.Contains(graph.edge[j].secondpick))
                        {
                            list.Add(graph.edge[j]);
                            hashset.Add(graph.edge[j].firstpick);
                            hashset.Add(graph.edge[j].secondpick);
                            break;
                        }
                        if (hashset.Contains(graph.edge[j].firstpick) && !hashset.Contains(graph.edge[j].secondpick))
                        {
